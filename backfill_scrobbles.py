    ...
    for team in teams:
        try:
            ...
            for page in range(1, BACKFILL_MAX_PAGES + 1):
                ...
                if "error" in data:
                    lines.append(f"[ERROR] Backfill {team['name']} | {team['lastfm_user']} | Last.fm API error {data.get('error')}: {data.get('message')}")
                    errors += 1
                    break
            ...
            lines.append(f"[OK] Backfill {team['name']} | {team['lastfm_user']} | insertados={inserted_for_team}")
        except Exception as e:
            errors += 1
            lines.append(f"[ERROR] Backfill {team['name']} | {team['lastfm_user']} | {str(e)}")
