...
body {{ margin:0; font-family:Arial, sans-serif; background:radial-gradient(circle at top left, rgba(59,130,246,.15), transparent 28%), radial-gradient(circle at top right, rgba(168,85,247,.12), transparent 25%), #061126; color:#e5e7eb; padding:24px; }}
...
table {{ width:100%; border-collapse:collapse; background:linear-gradient(180deg,rgba(17,28,52,.98),rgba(13,23,44,.98)); border:1px solid rgba(96,165,250,.16); border-radius:18px; overflow:hidden; margin-bottom:18px; }}
...
def render_monitor(cur):
    ...
    for t in teams:
        rows += f"""
        <tr><td><input type="checkbox" name="team_ids" value="{t['id']}"> {t['id']}</td><td>{t['name']}</td><td>{t['app_name']}</td><td>{t['lastfm_user']}</td><td>{badge(t['status'])}</td><td>{t['last_scrobble_at'] or '-'}</td><td>{t['idle_minutes'] if t['idle_minutes'] is not None else '-'}</td><td>{t['last_check_at'] or '-'}</td><td><div class="actions"><a class="btn btn-secondary" href="/edit-team-form?id={t['id']}">Editar</a><a class="btn btn-danger" href="/delete-team?id={t['id']}" onclick="return confirm('¿Borrar equipo?')">Borrar</a></div></td></tr>
        """
    if not rows:
    ...
    return f"""
    {filter_form("monitor")}
    <div class="card" style="margin-bottom:18px;"><div class="section-title">Agregar usuario Last.fm</div><form class="form-grid" method="GET" action="/seed-team"><div class="field"><label>Equipo</label><input name="name" placeholder="Box-01 01" required></div><div class="field"><label>App</label><select name="app"><option value="spotify">spotify</option><option value="apple">apple</option><option value="tidal">tidal</option><option value="youtube">youtube</option></select></div><div class="field"><label>Usuario Last.fm</label><input name="user" placeholder="equipoC01" required></div><button class="btn btn-primary">Agregar</button></form></div>
    <div class="card" style="margin-bottom:18px;"><div class="section-title">Carga masiva</div><form method="GET" action="/seed-batch"><div class="grid-2"><div class="field"><label>Prefijo</label><input name="prefix" placeholder="Box-03" required></div><div class="field"><label>App</label><select name="app"><option value="spotify">spotify</option><option value="apple">apple</option><option value="tidal">tidal</option><option value="youtube">youtube</option></select></div></div><div class="field" style="margin-top:12px;"><label>Usuarios separados por coma</label><textarea name="users" required></textarea></div><br><button class="btn btn-primary">Cargar varios</button></form></div>
...
def fetch_lastfm_user_created_at(lastfm_user):
    ...
    params = {"method": "user.getInfo", "user": lastfm_user, "api_key": LASTFM_API_KEY, "format": "json"}
...
def diagnose_one_team(cur, team):
    ...
    cur.execute("""
        ...
        ON CONFLICT (lastfm_user) DO UPDATE SET
            ...
            checked_at = NOW()
    """, (team_id, name, app_name, user, created_at, first_at, last_at, total, status, recommendation, error_message))
...
def render_historico(cur):
    ...
    <div class="card" style="margin-bottom:18px;"><div class="section-title">Control histórico Last.fm</div><div class="mini-row"><span>Último diagnóstico</span><strong>{s['last_check'] or '-'}</strong></div><div class="mini-row"><span>Regla</span><strong>No consulta Last.fm al abrir la vista; usa cache</strong></div></div>
    <div class="actions" style="margin-bottom:18px;"><a class="btn btn-primary" href="/refresh-lastfm-history">Actualizar diagnóstico Last.fm</a><a class="btn btn-secondary" href="/job-log?job=lastfm-history">Ver log</a></div>
    <form method="POST" action="/collect-all-selected"><div style="margin-bottom:12px;"><button class="btn btn-primary" type="submit">Collect All seleccionados</button></div><table><thead><tr><th>ID</th><th>Equipo</th><th>App</th><th>User</th><th>Creación Last.fm</th><th>Primer scrobble DB</th><th>Último scrobble DB</th><th>Total</th><th>Estado</th><th>Recomendación</th></tr></thead><tbody>{table_rows}</tbody></table></form>
    """
...
def render_analisis(cur):
    ...
    cur.execute(f"SELECT s.lastfm_user, COUNT(*) AS plays FROM scrobbles s WHERE {where} GROUP BY s.lastfm_user ORDER BY plays DESC LIMIT 25", params)
    users = cur.fetchall()
    user_rows = "".join([f"<tr><td>{r['lastfm_user']}</td><td>{r['plays']}</td></tr>" for r in users]) or '<tr><td colspan="2" class="muted">Sin datos</td></tr>'
    cur.execute(f"SELECT s.artist_name, COUNT(*) AS plays FROM scrobbles s WHERE {where} GROUP BY s.artist_name ORDER BY plays DESC LIMIT 10", params)
    artists = cur.fetchall()
    artist_html = "".join([f"<div class='mini-row'><span>{r['artist_name']}</span><strong>{r['plays']}</strong></div>" for r in artists]) or '<div class="muted">Sin datos</div>'
    cur.execute(f"""SELECT COALESCE(am.distributor, 'Sin distribuidora') AS distributor, COUNT(*) AS plays FROM scrobbles s LEFT JOIN artist_metadata am ON LOWER(am.artist_name) = LOWER(s.artist_name) WHERE {where} GROUP BY COALESCE(am.distributor, 'Sin distribuidora') ORDER BY plays DESC""", params)
    ...
    <div class="grid"><div class="card"><div class="label">Plays hoy</div><div class="value">{plays_today}</div></div><div class="card"><div class="label">Plays filtrados</div><div class="value">{plays_month}</div></div><div class="card"><div class="label">Promedio diario</div><div class="value">{avg_daily}</div></div><div class="card"><div class="label">Usuarios activos</div><div class="value">{len(users)}</div></div></div>
    <div class="card" style="margin-bottom:18px;"><div class="section-title">Reproducciones diarias</div><canvas id="playsChart"></canvas></div>
    <div class="card" style="margin-bottom:18px;"><div class="section-title">Reproducciones por distribuidora</div><canvas id="distributorChart"></canvas></div>
    <div class="grid-2"><div class="card"><div class="section-title">Top artistas</div>{artist_html}</div><div><div class="section-title">Reproducciones por usuario Last.fm</div><table><thead><tr><th>Usuario</th><th>Reproducciones</th></tr></thead><tbody>{user_rows}</tbody></table></div></div>
    <script>
    ...
    new Chart(document.getElementById('distributorChart'), {{type:'bar',data:{{labels:{json.dumps(distributor_labels)},datasets:[{{label:'Reproducciones por distribuidora',data:{json.dumps(distributor_values)},backgroundColor:'rgba(168,85,247,.35)',borderColor:'#a855f7',borderWidth:1}}]}},options:{{responsive:true,plugins:{{legend:{{labels:{{color:'#e5e7eb'}}}}}},scales:{{x:{{ticks:{{color:'#94a3b8'}}}},y:{{ticks:{{color:'#94a3b8'}}}}}}}}}});
    </script>
    """
...
def render_ganancias(cur):
    ...
    cur.execute(f"SELECT DATE(s.scrobble_time) AS play_day, LOWER(s.app_name) AS platform, COUNT(*) AS plays FROM scrobbles s WHERE {where} GROUP BY DATE(s.scrobble_time), LOWER(s.app_name) ORDER BY play_day ASC", params)
    daily = cur.fetchall()
    day_map = {}
    for r in daily:
        d = str(r["play_day"])[5:]
    ...
    cur.execute(f"SELECT s.artist_name, LOWER(s.app_name) AS platform, COUNT(*) AS plays FROM scrobbles s WHERE {where} GROUP BY s.artist_name, LOWER(s.app_name)", params)
    ...
    cur.execute(f"SELECT s.lastfm_user, LOWER(s.app_name) AS platform, COUNT(*) AS plays FROM scrobbles s WHERE {where} GROUP BY s.lastfm_user, LOWER(s.app_name)", params)
    ...
    <div class="card" style="margin-bottom:18px;"><div class="section-title">Ganancias por día</div><canvas id="gainChart"></canvas></div>
    <div class="grid-2"><div><div class="section-title">Ganancias por plataforma</div><table><thead><tr><th>Plataforma</th><th>Streams</th><th>Min</th><th>Max</th><th>Promedio</th></tr></thead><tbody>{platform_rows}</tbody></table></div><div class="card"><div class="section-title">Recomendación ejecutiva</div><div class="mini-row"><span>Empuja usuarios con más generación</span><strong>ROI</strong></div><div class="mini-row"><span>Cruza con Monitor Plays</span><strong>Meta 1K</strong></div><div class="mini-row"><span>Prioriza artistas con alza diaria</span><strong>Escala</strong></div></div></div>
    <div class="grid-2"><div><div class="section-title">Ganancias por artista</div><table><thead><tr><th>Artista</th><th>Ganancia</th></tr></thead><tbody>{artist_rows}</tbody></table></div><div><div class="section-title">Ganancias por usuario Last.fm</div><table><thead><tr><th>Usuario</th><th>Ganancia</th></tr></thead><tbody>{user_rows}</tbody></table></div></div>
    <script>new Chart(document.getElementById('gainChart'), {{type:'line',data:{{labels:{json.dumps(gain_labels)},datasets:[{{label:'Ganancia estimada',data:{json.dumps(gain_values)},borderColor:'#34d399',backgroundColor:'rgba(52,211,153,.15)',tension:.35,fill:true}}]}},options:{{responsive:true,plugins:{{legend:{{labels:{{color:'#e5e7eb'}}}}}},scales:{{x:{{ticks:{{color:'#94a3b8'}}}},y:{{ticks:{{color:'#94a3b8'}}}}}}}}}});</script>
    """
...
def render_monitor_plays(cur):
    ...
    for r in rows:
        plays = safe_int(r["plays"])
        faltan = 1000 - plays
        estado = '<span class="badge ok">🔥 Cerca</span>' if plays >= 900 else ('<span class="badge warn">⚠️ Push</span>' if plays >= 800 else '<span class="badge incident">🚨 Crítico</span>')
        table_rows += f"<tr><td>{r['artist_name']}</td><td>{r['track_name']}</td><td>{plays}</td><td>{faltan}</td><td>Dar {faltan} reproducciones</td><td>{estado}</td></tr>"
    if not table_rows:
        table_rows = '<tr><td colspan="6" class="muted">No hay canciones debajo de 1000.</td></tr>'
    return f"""
    {filter_form("monitor-plays")}
    <div class="grid-3"><div class="card"><div class="label">🔥 Cerca 900-999</div><div class="value green">{near}</div></div><div class="card"><div class="label">⚠️ Push 800-899</div><div class="value yellow">{push}</div></div><div class="card"><div class="label">🚨 Crítico &lt;800</div><div class="value red">{critical}</div></div></div>
    <div class="card" style="margin-bottom:18px;"><div class="section-title">Recomendación automática</div><div class="mini-row"><span>Primero empuja canciones 900-999</span><strong>Más fácil cerrar 1K</strong></div><div class="mini-row"><span>Luego 800-899</span><strong>Requieren presión</strong></div><div class="mini-row"><span>Menos de 800</span><strong>Solo si hay prioridad comercial</strong></div></div>
    <div class="section-title">Monitor Plays Pro</div><table><thead><tr><th>Artista</th><th>Canción</th><th>Plays</th><th>Faltan 1K</th><th>Recomendación</th><th>Estado</th></tr></thead><tbody>{table_rows}</tbody></table>
    """
...
def seed_team():
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""INSERT INTO teams(name, app_name, lastfm_user) VALUES(%s,%s,%s) ON CONFLICT(lastfm_user) DO NOTHING""", (name, app_name, user))
    conn.commit()
    cur.close()
    conn.close()
...
def seed_batch():
    ...
    for i, user in enumerate(users, start=1):
        cur.execute("""INSERT INTO teams(name, app_name, lastfm_user) VALUES(%s,%s,%s) ON CONFLICT(lastfm_user) DO NOTHING""", (f"{prefix} {i:02d}", app_name, user))
    conn.commit()
    cur.close()
    conn.close()
    return redirect("/?view=monitor")
...
def edit_team_form():
    ...
    return f"""<body style="background:#061126;color:white;font-family:Arial;padding:24px;"><h2>Editar equipo #{t['id']}</h2><form method="GET" action="/update-team" style="max-width:600px;"><input type="hidden" name="id" value="{t['id']}"><p>Equipo</p><input name="name" value="{t['name']}" style="width:100%;padding:12px;"><p>App</p><input name="app" value="{t['app_name']}" style="width:100%;padding:12px;"><p>User</p><input name="user" value="{t['lastfm_user']}" style="width:100%;padding:12px;"><br><br><button>Guardar</button> <a href="/?view=monitor" style="color:#c4b5fd;">Volver</a></form></body>"""
...
def update_team():
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("UPDATE teams SET name=%s, app_name=%s, lastfm_user=%s WHERE id=%s", (request.args.get("name"), request.args.get("app"), request.args.get("user"), request.args.get("id")))
    conn.commit()
    cur.close()
    conn.close()
    return redirect("/?view=monitor")
...
def collect_all_selected():
    ...
    if not team_ids:
        return '<body style="background:#061126;color:white;font-family:Arial;padding:24px;"><h2>No seleccionaste equipos</h2><a style="color:#93c5fd;" href="/?view=monitor">Volver</a></body>'
    ids_text = ",".join(team_ids)
    start_logged_job("backfill_scrobbles.py", "collect-all-selected", extra_env={"TEAM_IDS": ids_text})
...
def job_log():
    ...
    if not os.path.exists(log_path):
        ...
        with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
            content = f.read()
    return f"""<html><head><meta http-equiv="refresh" content="5"><title>WatchEagle Job Log</title><style>body {{background:#061126;color:white;font-family:Arial;padding:24px;}} pre {{background:#020617;color:#d1d5db;border:1px solid #334155;border-radius:14px;padding:18px;white-space:pre-wrap;}} a {{color:#93c5fd;}}</style></head><body><h1>WatchEagle Job Log</h1><p><a href="/">Volver al dashboard</a></p><p>Se actualiza cada 5 segundos.</p><pre>{content}</pre></body></html>"""
...
def init_artist_metadata():
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""CREATE TABLE IF NOT EXISTS artist_metadata (id SERIAL PRIMARY KEY, artist_name TEXT UNIQUE NOT NULL, author TEXT, distributor TEXT)""")
    for artist, author, distributor in ARTIST_METADATA:
        cur.execute("""INSERT INTO artist_metadata (artist_name, author, distributor) VALUES (%s, %s, %s) ON CONFLICT (artist_name) DO UPDATE SET author = EXCLUDED.author, distributor = EXCLUDED.distributor""", (artist, author, distributor))
    conn.commit()
    cur.close()
    conn.close()
