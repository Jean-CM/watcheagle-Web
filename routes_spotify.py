...
def save_token(cur, data):
    cur.execute("""INSERT INTO spotify_tokens (id,access_token,refresh_token,expires_at,updated_at)
    VALUES (1,%s,%s,NOW()+(%s || ' seconds')::interval,NOW())
    ON CONFLICT (id) DO UPDATE SET access_token=EXCLUDED.access_token,
    refresh_token=COALESCE(EXCLUDED.refresh_token, spotify_tokens.refresh_token),
    expires_at=EXCLUDED.expires_at, updated_at=NOW()""", (data.get('access_token'), data.get('refresh_token'), safe_int(data.get('expires_in'),3600)))
...
def search_uri(tok, artist, track):
    queries=[f'track:"{track}" artist:"{artist}"', f'{artist} {track}', f'{track} {artist}', track]
    first=None
    for q in queries:
        ...
        first = first or items[0].get('uri')
        for item in items:
            t=norm(item.get('name')); arts=[norm(a.get('name')) for a in item.get('artists',[])]
            if (norm(track) in t or t in norm(track)) and any(norm(artist) in a or a in norm(artist) for a in arts):
                return item.get('uri')
    return first
...
def candidates(cur, strategy):
    ...
    title='WatchEagle Playlist'; having='COUNT(*) < 1000'; order='COUNT(*) ASC'
    if strategy=='under600': title='WatchEagle - Prioridad Alta <600'; having='COUNT(*) < 600'
    elif strategy=='under900': title='WatchEagle - Empuje <900'; having='COUNT(*) < 900'
    elif strategy=='near1k': title='WatchEagle - Cerca de 1K'; having='COUNT(*) >= 900 AND COUNT(*) < 1000'; order='COUNT(*) DESC'
    elif strategy=='random8h': title='WatchEagle - Random 8H'; order='RANDOM()'
...
def create_playlist(cur, strategy):
    tok=token(cur); profile=me(tok); title, rows=candidates(cur,strategy)
    if not rows: raise Exception(f'No hay canciones candidatas para {strategy}. Prueba <600, <900 o random8H.')
    uris=[]; nf=[]; seen=set()
    for x in rows:
    ...
    r=requests.post(f'{API}/me/playlists',headers=headers(tok),json={'name':name,'description':'Creada por WatchEagle.','public':not SPOTIFY_PLAYLIST_PRIVATE},timeout=20)
    if r.status_code>=400: raise Exception(f'Error creando playlist: {r.status_code} {r.text[:300]}')
    playlist=r.json(); added=0
    for i in range(0,len(uris),100):
        chunk=uris[i:i+100]
        rr=requests.post(f'{API}/playlists/{playlist["id"]}/tracks',headers=headers(tok),json={'uris':chunk},timeout=30)
        if rr.status_code>=400: raise Exception(f'Error agregando tracks: {rr.status_code} {rr.text[:250]}')
        added+=len(chunk)
...
def render_spotify(cur):
    ensure_tables(cur); cur.execute('SELECT * FROM spotify_tokens WHERE id=1'); row=cur.fetchone()
    cur.execute('SELECT playlist_name,playlist_url,strategy,tracks_added,tracks_not_found,created_at FROM spotify_playlist_runs ORDER BY created_at DESC LIMIT 12'); runs=cur.fetchall()
...
def register_spotify_routes(app, get_conn, base_page):
    ...
    app.add_url_rule('/spotify/callback','spotify_callback',callback)
    app.add_url_rule('/callback','spotify_callback_root',callback)
    app.add_url_rule('/spotify-callback','spotify_callback_alt',callback)
